...
The citizens of Codeville seem pleased with all the upgrades your administration has been making to the local infrastructure, and they want more! 
...
? that returns the coordinates of an available parking spot for the vehicle, 
    ...
    if ( available ) {
      for ( let x of y ) {
        ...
        if (vehicle == "motorcycle" && (x === "R" || x === "S" || x === "M")) {
          return [X,Y];
        } else if (vehicle == "small" && (x === "R" || x === "S") ) {
          return [X,Y]; 
        } else if (vehicle == "regular" && (x === "R") ) {
          return [X,Y];
        };
      }
    }
