...
You've decided that you want to install air pollution sensors around the city to monitor air quality and identify problem areas. 
...
It might be useful to start by creating a variable 
to keep track of how many dirty samples we find as we loop through the array of samples. 
We can increment or add to this number using the ++ operator.
Once you know how many dirty samples there are, 
we just need to do some simple math to determine if it exceeds the threshold.
*/

const checkAir = function (samples, threshold) {

  let dirtyCount = samples.filter((word) => word === "dirty").length;
  const proportionOfDirty = (count) => { return count/samples.length };

  if (proportionOfDirty(dirtyCount) >= threshold) {
    return "Polluted";
  } else {
    return "Clean";
  }
